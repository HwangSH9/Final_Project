...
public interface MissingMapper {
	...
	public List<MissingVO> getList();
	
	// 앞서 작성한 Criteria 타입을 파라미터로 사용하는
	// getListWithPaging() 메서드를 작성합니다.
	public List<MissingVO> getListWithPaging(Criteria cri);
	
	// create(insert) 처리를 위한 insert()와 insertSelectKey() 추상 메서드 정의
	public void insert(MissingVO board);
	
	public void insertSelectKey(MissingVO board);
	
	// read(select) 처리를 위한 read() 추상 메서드 정의
	public MissingVO read(Long bno);
	...
	public int update(MissingVO board);
