...
import org.zerock.service.MissingReplyService;
...
public class MissingReplyController {
	...
	public ResponseEntity<String> create(@RequestBody MissingReplyVO vo){
		log.info("ReplyVO~ ♥-♥ : " + vo);
	...
	public ResponseEntity<MissingReplyPageDTO> getList(@PathVariable("page") int page, @PathVariable("bno") Long bno){
	...
	@GetMapping(value = "/{rno}", produces = {
			MediaType.APPLICATION_XML_VALUE,
			MediaType.APPLICATION_JSON_UTF8_VALUE})
	public ResponseEntity<MissingReplyVO> get(@PathVariable("rno") Long rno){
...
// STS에서 Console창에 처리된 메시지를 확인합니다.
	...
	// Page732 소스 코딩할 때 아래 1줄 소스 주석 처리
	// public ResponseEntity<String> remove(@PathVariable("rno") Long rno){
	// Page732 아래 소스 코딩 1줄 추가 : @RequestBody가 적용되어 JSON으로 된 데이터를
	//                           받도록 수정 처리 되었습니다.
	// 실행 확인은 브라우저에서는 JSON 데이터를 전송하고, ReplyController에서는 로그를
	// 통해서 정상적으로 동작하는 확인해 봅니다.
	public ResponseEntity<String> remove(@RequestBody MissingReplyVO vo, @PathVariable("rno") Long rno){
	...
	// Page734 아래 1줄 소스 코딩 추가 : 시큐리티 @PreAuthorize 어노테이션 추가
	@PreAuthorize("principal.username == #vo.replyer")
	@RequestMapping(method = {RequestMethod.PUT, RequestMethod.PATCH}, value = "/request/{rno}", consumes = "application/json", produces = {
			MediaType.TEXT_PLAIN_VALUE})
	public ResponseEntity<String> modify(@RequestBody MissingReplyVO vo, @PathVariable("rno") Long rno){
		vo.setRno(rno);
		log.info("rno : ▼-▼/ : " + rno);
		log.info("modify : ＆-＆/ : " + vo);
